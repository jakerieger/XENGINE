...
string INIReader::ParseErrorMessage() const {
    ...
    // without a value.
    if (_error > 0) { return "parse error on line " + std::to_string(_error) + "; missing ']' or '='?"; }
...
bool INIReader::GetBoolean(const string& section, const string& name, bool default_value) const {
    ...
    std::transform(valstr.begin(), valstr.end(), valstr.begin(), [](const unsigned char& ch) {
...
std::vector<string> INIReader::Sections() const {
    ...
    for (std::map<string, string>::const_iterator it = _values.begin(); it != _values.end(); ++it) {
        ...
        if (pos != string::npos) { sectionSet.insert(it->first.substr(0, pos)); }
    }
