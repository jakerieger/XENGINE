...
namespace Xen::PAK {
    ...
    void PakFileSource::OpenAndReadTable() {
        ...
        if (!_FileStream) {
            throw std::runtime_error(
              Pak_MakeExceptionStr("failed to open pak file: " + _PakPath.string()));
        }
        ...
        if (!_FileStream) {
            throw std::runtime_error(
              Pak_MakeExceptionStr("failed to seek to table offset in: " + _PakPath.string()));
        }

        _Salt = Header.Salt;

        if (ComputeKeyCheck(_Salt, GetBuiltInKeySchedule()) != Header.KeyCheck) {
            throw CryptoException(Pak_MakeExceptionStr(
    ...
    AssetBuffer PakFileSource::LoadFull(const AssetID ID) {
        ...
        if (It == _Table.end()) {
            throw std::runtime_error(
              Pak_MakeExceptionStr("unknown asset ID: " + std::to_string(ID.Value)));
        }
        ...
        if (!_FileStream) {
            throw std::runtime_error(
              Pak_MakeExceptionStr("seek failed for asset ID " + std::to_string(ID.Value)));
        }

        std::vector<u8> Stored(Entry.CompressedSize);
        if (Entry.CompressedSize > 0 &&
            !_FileStream.read(RCAST<char*>(Stored.data()), Entry.CompressedSize)) {
            throw std::runtime_error(
              Pak_MakeExceptionStr("read failed for asset ID " + std::to_string(ID.Value)));
        }

        // Compress and then encrypt (decrypt and then decompress)
        AesCtrXcryptInPlace(Stored,
                            GetBuiltInKeySchedule(),
                            DeriveNonce(Entry.ID, _Salt, GetBuiltInKeySchedule()));

        auto Data = std::make_unique<u8[]>(Entry.UncompressedSize);

        if (Entry.Codec == PakCodec::None) {
            if (Entry.UncompressedSize > 0) {
                std::memcpy(Data.get(), Stored.data(), Entry.UncompressedSize);
            }
        } else {
            const std::unique_ptr<ICodec> Codec = CreateCodec(Entry.Codec);
            if (!Codec) { throw std::runtime_error(Pak_MakeExceptionStr("no compressor for codec")); }

            Codec->Decompress(Stored.data(),
