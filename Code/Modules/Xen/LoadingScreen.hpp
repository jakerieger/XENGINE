...
//
// The engine's built-in loading screen: a background, a spinner and a progress
// bar, drawn straight into the swap chain by its own tiny pipeline - so it needs
...
namespace Xen {
    ...
    struct LoadingProgress {
        /// 0..1: finished assets over total. Not smoothed - LoadingScreen
        /// eases its own bar toward it.
        f32 Fraction {0.0f};
        size_t Done {0};
        size_t Total {0};
    ...
    public:
        ...
        struct Config {
            ...
            Color Track {0.14f, 0.15f, 0.185f, 1.0f};
            Color Accent {0.33f, 0.62f, 1.0f, 1.0f};
        ...
        /// @brief Restarts the bar's ease and the spinner's angle - call when
        /// a new load begins to show.
        void Reset();
        ...
        /// @brief Background, spinner and progress bar, into the current frame
        /// (between BeginFrame and EndFrame). DeltaTime advances the spinner
        /// and the bar's ease.
