// Bakes the diffuse irradiance map: each output texel is the source
// environment convolved with a cosine lobe around that texel's direction, so a
// diffuse surface reads its whole-hemisphere lighting with one texture sample
// instead of integrating it per pixel. Drawn once by EnvironmentBaker into a
// small (64x32) map - irradiance is so low-frequency that more resolution
...
// samples are taken from the source's mip pyramid at a level chosen from each
// sample's pdf, so a bright small feature (a sun disc) is spread smoothly
// instead of showing up as speckle.

#include "Include/Common.hlsli"
#include "Include/Fullscreen.hlsli"

cbuffer BakeParams : register(b0) {
    float4 Params;  // y = SampleCount
};
